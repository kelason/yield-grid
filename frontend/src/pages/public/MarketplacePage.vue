...
<template>
  <div class="space-y-6">
    ...
    <div class="rounded-2xl bg-gradient-to-r from-farm-600 to-farm-800 p-6 text-white shadow-lg">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          ...
          <p class="text-farm-200 text-sm mt-1">Secure your supply directly from Filipino farmers at a fixed price.</p>
        </div>
        <span class="inline-flex items-center gap-1.5 bg-white/10 rounded-lg px-3 py-1.5 text-sm font-medium self-start sm:self-auto">
          🌾 Forward Contracts
        </span>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showCheckoutPanel" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        <!-- Backdrop -->
        <div
        ...
        <!-- Modal Panel -->
        <div class="relative w-full max-w-3xl max-h-[90vh] flex flex-col bg-white rounded-2xl shadow-2xl overflow-hidden transform transition-all">
          
          <!-- Header -->
          <div class="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100 z-10">
          ...
          <div class="flex-1 overflow-y-auto p-0 bg-gray-50 relative">
            <div v-if="marketStore.loading.details" class="flex justify-center items-center h-64">
              <svg class="animate-spin h-8 w-8 text-farm-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
