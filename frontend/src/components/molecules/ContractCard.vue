...
<template>
  <div class="bg-white overflow-hidden shadow-sm rounded-lg border border-gray-100 hover:shadow-md transition-shadow duration-200 hover:border-farm-200 transform hover:-translate-y-0.5 transition-all duration-300 flex flex-col h-full">
    <div class="h-1 bg-gradient-to-r from-farm-500 to-farm-600 w-full"></div>
    
    <div class="p-5 flex-grow flex flex-col">
      <div class="flex justify-between items-start mb-4">
        <div>
          <h3 class="text-lg font-bold text-gray-900 leading-tight mb-1 line-clamp-2" :title="contract.title">
            {{ contract.title }}
          </h3>
      ...
      <div class="space-y-2 mb-6 flex-grow">
        <div class="flex items-center text-sm text-gray-600">
          ...
          <span>Harvest: <span class="font-medium text-gray-900">{{ contract.estimated_harvest_date }}</span></span>
        </div>
        ...
        <div class="flex items-center text-sm text-gray-600">
          ...
          <span class="truncate">{{ contract.farmer?.location || contract.farmer?.farm_name || 'Location' }}</span>
        </div>
      </div>
