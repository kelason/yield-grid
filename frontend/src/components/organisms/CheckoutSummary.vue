...
import { CalendarIcon, MapPinIcon, UserIcon, CheckCircleIcon, InformationCircleIcon } from '@heroicons/vue/24/outline'
...
<template>
  <div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-xl overflow-hidden border border-white max-w-2xl mx-auto">
    ...
    <div class="p-6 sm:p-8">
      ...
      <p class="text-gray-500 text-sm mb-8">Please confirm the details of this forward contract before proceeding to payment.</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 text-sm">
          <div class="flex items-start">
            ...
            <div>
              ...
              <span class="text-gray-600">{{ contract.crop_name }} ({{ contract.quantity_kg }}kg)</span>
            </div>
          </div>
          ...
          <div class="flex items-start">
            ...
            <div>
              ...
              <span class="text-gray-600">{{ contract.farmer?.location || contract.farmer?.farm_name }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="flex flex-col sm:flex-row justify-between items-end mb-8">
        ...
        <div class="text-right w-full sm:w-auto">
          <div class="text-sm text-gray-500 mb-1">Total Amount (<PriceTag :amount="pricePerKg" size="sm" />/kg)</div>
          <PriceTag :amount="contract.total_price" :currency="contract.currency" size="lg" class="text-farm-700" />
        </div>
      </div>
      
      <div class="bg-blue-50 text-blue-800 p-4 rounded-lg flex items-start mb-8 text-sm border border-blue-100">
        <InformationCircleIcon class="h-5 w-5 mr-3 flex-shrink-0 text-blue-500 mt-0.5" />
        <p>You will be redirected to PayMongo's secure checkout page to complete this transaction. The contract will be reserved for 30 minutes.</p>
      </div>
