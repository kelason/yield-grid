...
const statusConfig = computed(() => {
  const map = {
    ...
    reserved: { classes: 'bg-yellow-100 text-yellow-800 border border-yellow-200', label: 'Reserved' },
    ...
    pending: { classes: 'bg-blue-50 text-blue-700 border border-blue-200', label: 'Pending Payment' },
    completed: { classes: 'bg-green-100 text-green-800 border border-green-200', label: 'Completed' },
    failed: { classes: 'bg-red-100 text-red-800 border border-red-200', label: 'Failed' },
  }
  
  return map[props.status.toLowerCase()] || { classes: 'bg-gray-100 text-gray-800 border border-gray-200', label: props.status }
})
</script>
